...
@router.post(f"{system.path}/likeGJItem211.php", response_class=PlainTextResponse, tags=['Misc'])
async def like_item(
    itemID: str = Form(),
    ...
    if liketype == 1:
        if like == 1:
            ...
            like = await LevelObject(service=service, db=db).dislike(accountID=accountID)
            if like['status'] == "ok":
                info("Dislike")
                return "1"
