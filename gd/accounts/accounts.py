...
from utils.crypt import bcrypt_hash,base64_decode, checkValidGJP
...
@router.post(f'{path}/uploadGJMessage20.php', response_class=PlainTextResponse)
async def uploadMessage(
    ...
    if await checkValidGJP(id=accountID, gjp=gjp, db=db) == True:
        message = await UserService.upload_message(db=db,
                                   authorID=accountID,
...
@router.post(f'{path}/getGJMessages20.php', response_class=PlainTextResponse)
