...
from utils.crypt import bcrypt_hash, base64_decode, checkValidGJP, xor_cipher
...
@router.post(f"{system.path}/accounts/registerGJAccount.php", response_class=PlainTextResponse,)
...
@router.post(f"{system.path}/accounts/loginGJAccount.php", response_class=PlainTextResponse)
async def login(
...
@router.post(f"{system.path}/uploadGJMessage20.php", response_class=PlainTextResponse, include_in_schema=False)
async def uploadMessage(
    db: AsyncSession = Depends(get_db),
    gjp: str = Form(),
    accountID: str = Form(),
    toAccountID: str = Form(),
    subject: str = Form(),
    body: str = Form(),
):
    if await checkValidGJP(id=accountID, gjp=gjp, db=db):
        message = await UserService.upload_message(
            db=db,
            authorID=accountID,
