...
def updateGJUserScore22(accountID: str = Form(),stars: str = Form(),
    ...
    iconkit = str({"color1": int(color1), "color2":  int(color2), "accBall":  int(accBall), "accBird":  int(accBird), "accDart":  int(accDart), "accGlow":  int(accGlow), "accIcon":  int(accIcon), "accShip":  int(accShip), "accRobot":  int(accRobot), "accSpider":  int(accSpider), "accExplosion":  int(accExplosion)}).replace("'",'"')
